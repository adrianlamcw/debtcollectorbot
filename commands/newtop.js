...
async function getTopDebtors(message, client, database, count = 10) {
    const statement = `
        ...
        ORDER BY total DESC
        LIMIT ${count};`;
    database.query(statement, async function (error, rows) {
        let text = "Top Debt Holders\n";
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const id = row.borrower_id;
            const amount = row.total;
            const name = await newutils_1.getUsername(client, id);
